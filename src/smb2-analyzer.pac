...
connection SMB2_Conn(bro_analyzer: BroAnalyzer) {
	upflow = SMB2_Flow(true);
	downflow = SMB2_Flow(false);
};

flow SMB2_Flow(is_orig: bool) {
	...
	function proc_smb2_negotiate_response(header: SMB2_Header, res: SMB2_negotiate_response) : bool
		...
		bro_event_smb2_negotiate_response(connection()->bro_analyzer(),
