...
type SMB_PDU(is_orig: bool) = record {
	...
	header      : SMB_Header;
	message     : SMB_Message(header, is_orig);
} &byteorder = littleendian &length = nbss.message_length+4;

type SMB_Message(header: SMB_Header, is_orig: bool) = case is_orig of {
	true    ->  request   : SMB_Message_Request(header, is_orig);
	false   ->  response  : SMB_Message_Response(header, is_orig);
};

type SMB_Message_Request(header: SMB_Header, is_orig: bool) = case header.command of {
...
type SMB_Message_Response(header: SMB_Header, is_orig: bool) = case header.command of {
...
type SMB_read_andx_response(header: SMB_Header) = record {
	...
	data             : bytestring &chunked &length = data_len;
...
type SMB_write_andx_request(header: SMB_Header) = record {
	...
	byte_count    : uint16;
	pad           : uint8;
	data          : bytestring &chunked &length=data_len; # TODO: this should be done chunk-wise
} &let {
	data_len = data_len_high * 0x10000 + data_len_low;
};
